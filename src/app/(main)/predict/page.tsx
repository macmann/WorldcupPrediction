...
import { PredictDateTabs, type PredictDateTabItem } from "@/components/PredictDateTabs";
import { addAppDays, formatAppDateTime, formatAppDate } from "@/lib/dateTime";
import { dateTabs, matchDateKey, selectedDateTab } from "@/lib/predictTabs";
...
export default async function MatchCenter({ searchParams }: MatchCenterProps) {
  ...
  const selectedDate = selectedDateTab(dates, requestedDate, requestedGroup, now);
  ...
  const dateTabItems: PredictDateTabItem[] = dates.map((tabDate) => ({
    date: tabDate,
    label: dateTabLabel(tabDate, now, t),
    matchCount: allMatches.filter((match) => matchDateKey(match.kickoffTime) === tabDate).length
  }));
      ...
      <details className="rounded-3xl bg-white/80 p-4 shadow-sm">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div>
            ...
            <PredictDateTabs dates={dateTabItems} selectedDate={selectedDate} streamParam={streamParam} emptyLabel={t("predict.fixturesSyncing")} />
          </div>
          <div>
