...
export async function PATCH(request: Request) {
  try {
    ...
    const settings = await prisma.appSetting.upsert({
      where: { id: 1 },
      create: { id: 1, announcementText: input.announcementText ?? null, bannerImageUrl: input.bannerImageUrl ?? null, loginBackgroundImageUrl: input.loginBackgroundImageUrl ?? null, maintenanceMode: input.maintenanceMode ?? false },
      update: {
