...
export function OutrightPicksCard({ canEdit }: { canEdit: boolean }) {
  ...
  return (
    <Card>
      ...
      <p className="mt-2 text-sm font-semibold text-slate-500">Choose your Champion, Best Player, and Best Goalkeeper from live tournament data after signing in.</p>
      ...
      {!completed && <button type="button" onClick={submit} disabled={!canSubmit} className="mt-5 w-full rounded-2xl bg-emerald-600 py-4 font-black text-white shadow-lg shadow-emerald-600/20 transition active:scale-[0.98] disabled:bg-slate-300">{isPending ? "Saving picks…" : liveCanEdit ? "Save tournament picks" : "Picks are locked"}</button>}
    </Card>
  );
}
