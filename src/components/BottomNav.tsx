...
export function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 mx-auto w-full max-w-[430px] border-t border-slate-200 bg-white/95 px-3 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-16px_40px_rgba(15,23,42,0.08)] backdrop-blur">
      <div className="grid grid-cols-6 gap-1 text-center text-[11px] font-black text-slate-500">
        {tabs.map((tab) => {
          const active = isActive(pathname, tab.href);
          const Icon = tab.icon;
          return (
            <Link key={tab.href} href={tab.href} className={`group relative rounded-2xl px-1 py-2 transition active:scale-95 ${active ? "bg-emerald-50 text-emerald-700" : "hover:bg-slate-50 hover:text-indigo-700"}`}>
