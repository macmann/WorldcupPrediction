...
test("maps WC26 group names with aliases and diacritics to flag URLs", () => {
  ...
  assert.equal(countryCodeFromName("Côte d'Ivoire"), "CI");
  assert.equal(countryCodeFromName("Cape Verde Islands"), "CV");
  assert.equal(countryCodeFromName("Cabo Verde"), "CV");
  assert.equal(countryCodeFromName("IR Iran"), "IR");
